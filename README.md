<p align="center">
  <a href="https://rettecnologia.org">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=28&duration=3000&pause=1000&color=FFFFFF&background=0A0A0A&center=true&vCenter=true&multiline=true&repeat=true&width=700&height=100&lines=%F0%9F%93%9A+Crescimento+%26+Aprendizagem;RET+Tecnologia+%E2%80%A2+L%26D+Framework+2026" alt="Header" />
  </a>
</p>

<p align="center">
  <a href="https://rettecnologia.org"><img src="https://img.shields.io/badge/🌐_Site-rettecnologia.org-0A0A0A?style=for-the-badge" alt="Website" /></a>
  <a href="https://www.linkedin.com/company/ret-consultoria/"><img src="https://img.shields.io/badge/LinkedIn-RET_Consultoria-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="mailto:contato@rettecnologia.org"><img src="https://img.shields.io/badge/Email-contato@rettecnologia.org-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
  <a href="https://github.com/Ret-Consultoria"><img src="https://img.shields.io/badge/GitHub-Ret--Consultoria-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Framework-L%26D_2026-blueviolet?style=flat-square" alt="Framework" />
  <img src="https://img.shields.io/badge/ROI-218%25_Revenue%2FEmployee-success?style=flat-square" alt="ROI" />
  <img src="https://img.shields.io/badge/KPI-DORA_Metrics-blue?style=flat-square" alt="DORA" />
  <img src="https://img.shields.io/badge/License-MIT-green?style=flat-square" alt="License" />
</p>

<br/>

<p align="center">
  <strong>Empresas com L&D robusto reportam 218% mais receita por colaborador e 57% mais retenção.<br/>Na RET, aprendizagem contínua não é benefício — é infraestrutura crítica de operação.</strong>
</p>

---

## 📑 Sumário

- [🎯 Por Que Este Compromisso Existe](#-por-que-este-compromisso-existe)
- [🏗️ Os 5 Pilares de Aprendizagem](#️-os-5-pilares-de-aprendizagem)
- [📊 Dashboard de Impacto](#-dashboard-de-impacto)
- [📚 Referências e Frameworks](#-referências-e-frameworks)
- [🤝 Como Contribuir](#-como-contribuir)
- [🔗 Nossos Outros Compromissos](#-nossos-outros-compromissos)

---

## 🎯 Por Que Este Compromisso Existe

> *"A meia-vida de uma habilidade técnica hoje é de apenas 5 anos. Se o time não evolui continuamente, seu produto está morto antes do próximo release."*

Na **RET Tecnologia**, entendemos que o capital humano é o único ativo que se valoriza com o uso. Não tratamos desenvolvimento profissional como benefício corporativo genérico — tratamos como **investimento estratégico com ROI mensurável**.

### 📈 Os dados são inequívocos:

| Indicador (Pesquisa Global 2026) | Impacto | Fonte |
|---|---|---|
| Receita por colaborador em empresas com L&D forte | **+218%** | ATD Research |
| Ganho de produtividade via upskilling contínuo | **+39%** | WEF Future of Jobs |
| Melhoria na retenção de talentos | **+57%** | LinkedIn Workforce Learning |
| Redução de custos vs. contratação externa | **até 53%** | Deloitte Human Capital |
| Custo de substituição de engenheiro sênior | **150-200% do salário** | SHRM Benchmarks |
| Trabalhadores que precisam de reskilling até 2027 | **60%** | World Economic Forum |

> Cada engenheiro que sobe de nível eleva a qualidade de cada linha de código entregue aos nossos clientes. É um investimento com retorno **comprovado**.

---

## 🏗️ Os 5 Pilares de Aprendizagem

### 1. 📖 Skills-First Learning Paths

Não deixamos crescimento profissional ao acaso. Cada cargo possui trilhas de aprendizagem com **cursos, livros, certificações e projetos práticos** mapeados por senioridade, alinhadas ao conceito de **capability velocity** — a velocidade com que a organização constrói capacidades estratégicas.

<details>
<summary><strong>📋 Trilha: Engenheiro(a) Backend Pleno</strong></summary>

<br/>

| Módulo | Recurso | Tipo | Proficiency Level |
|--------|---------|------|-------------------|
| **Arquitetura Avançada** | Padrões de Microsserviços | Curso | Intermediate → Advanced |
| | *Building Microservices* — Sam Newman | Livro | Referência |
| | Refatorar monolito → microsserviços | Projeto Prático | Applied |
| **DevOps & Cloud Native** | CKAD (Kubernetes) | Certificação | Industry Standard |
| | CI/CD com GitHub Actions → AWS/GCP | Tutorial | Applied |
| | Terraform IaC + GitOps | Lab | Advanced |
| **Security Engineering** | OWASP Top 10 + Secure Coding | Workshop | Required |
| | Threat Modeling (STRIDE) | Hands-on | Intermediate |
| **Soft Skills & Leadership** | Liderança Técnica e Mentoria | Workshop | Essential |
| | Comunicação Assíncrona Eficaz | Artigo + Prática | Applied |
| | Pensamento Crítico e Estratégico | Exercícios | 🔥 Prioridade #1 2026 |

</details>

<details>
<summary><strong>📋 Trilha: Engenheiro(a) de Segurança</strong></summary>

<br/>

| Módulo | Recurso | Tipo | Proficiency Level |
|--------|---------|------|-------------------|
| **Red Team Foundations** | PTES + OWASP Testing Guide | Estudo Dirigido | Foundational |
| **Análise de Vulnerabilidades** | CVE Analysis + Nuclei Templates | Prática | Intermediate |
| **OSINT & Recon** | Subfinder, Amass, Shodan | Lab | Applied |
| **Exploit Development** | PoC Writing + Responsible Disclosure | Projeto | Advanced |
| **Certificação** | OSCP / CEH / PNPT | Certificação | Industry Standard |
| **AI Security** | AI Red Teaming + Prompt Injection | Workshop | 🔥 Emergente 2026 |

</details>

<details>
<summary><strong>📋 Trilha: AI/ML Engineer</strong></summary>

<br/>

| Módulo | Recurso | Tipo | Proficiency Level |
|--------|---------|------|-------------------|
| **Foundations** | Linear Algebra + Probability | Curso | Foundational |
| **ML Engineering** | Scikit-learn → PyTorch pipeline | Projeto | Intermediate |
| **LLM & RAG** | LangChain, Vector DBs, Fine-tuning | Lab | Advanced |
| **MLOps** | MLflow, Model Registry, A/B Testing | Hands-on | Applied |
| **AI Safety** | Red Teaming LLMs, Alignment | Workshop | 🔥 Critical 2026 |
| **Certificação** | AWS ML Specialty / GCP ML Engineer | Certificação | Industry Standard |

</details>

---

### 2. 🧑‍🏫 Mentoria Bidirecional Estruturada

Profissionais seniores guiam o desenvolvimento de colegas em sessões 1:1 estruturadas. A mentoria é **bidirecional** — quem ensina, aprende. Quem aprende, acelera.

Seguimos o modelo de **mentorship programs** citado por Google (Googler-to-Googler) e Siemens como best practice para retenção:

- 📅 Sessões quinzenais de 45 minutos com **agenda documentada**
- 🎯 Pareamento mentor-mentorado por stack e objetivos de carreira
- 📊 Feedback contínuo e plano de evolução com **milestones trimestrais**
- 🔄 Reverse mentoring: juniors ensinam seniors sobre novas tecnologias

### 3. 💡 Acesso a Recursos de Ponta + Budget Dedicado

Cada membro do time tem acesso ilimitado a plataformas de ensino de classe mundial e um **budget pessoal** para investir em sua evolução:

| Plataforma | Uso | Diferencial |
|-----------|-----|-------------|
| **Alura** | Trilhas em português, infra a cloud | Ecossistema BR |
| **Pluralsight** | Role-based paths + Skill IQ assessments | Skill tracking nativo |
| **Coursera** | Especializações Stanford, Google, AWS | Universidades tier-1 |
| **O'Reilly** | Livros técnicos, early releases, live events | Catálogo referência |
| **Cloud Academy** | Labs interativos AWS/GCP/Azure | Hands-on real |
| **Budget pessoal** | Livros, conferências, certificações à escolha | **R$ 3.000+/ano** |

### 4. 💬 Guildas de Tecnologia (Comunidades de Prática)

Comunidades internas organizadas por especialidade — espaços seguros para inovação coletiva, modeladas no formato de **Communities of Practice** usado por Spotify e ThoughtWorks.

| Guilda | Foco | Key Topics 2026 |
|--------|------|-----------------|
| ⚙️ **Backend** | Arquitetura, APIs, microsserviços | Event-driven, gRPC, DDD |
| 🎨 **Frontend** | Performance, UX, design systems | Server Components, Edge |
| 🔧 **DevOps** | CI/CD, IaC, observabilidade | GitOps, DORA metrics |
| 🔒 **Security** | SAST/DAST, threat modeling, red team | AI Security, Supply Chain |
| 🤖 **AI & ML** | LLMs, RAG, AI Engineering | AI Quotient organizacional |

> **Atividades:** Tech talks semanais · Pair programming · Code reviews cruzados · Hackathons internos · Lightning talks de 10 min

### 5. 📐 Medição Baseada em DORA Metrics + L&D KPIs

Não medimos apenas *horas de treinamento* (vanity metric). Medimos **impacto no negócio** alinhado com DORA Metrics e Capability Velocity:

| Categoria | KPI | O que mede |
|-----------|-----|-----------|
| 🚀 **Velocidade** | Deployment Frequency | Frequência de deploys após upskilling |
| ✅ **Qualidade** | Change Failure Rate | Redução de falhas após treinamento |
| 🧠 **Competência** | Time to Competency | Tempo para atingir proficiency em nova stack |
| 🎯 **Retenção** | Skills Gap Closure Rate | Velocidade de fechamento de gaps |
| 📊 **Aplicação** | Return on Expectation (ROE) | Alinhamento entre resultado e expectativa |

---

## 📊 Dashboard de Impacto

| Indicador | Meta | Benchmark Mercado |
|-----------|------|-------------------|
| ⏱️ Horas de aprendizagem / engenheiro / mês | **8h+** | Média mercado: 4h |
| 🏆 Certificações conquistadas / ano | **2+ por pessoa** | Média: 0.8 |
| 🤝 Sessões de mentoria / trimestre | **6+ por dupla** | Best practice: 6 |
| 👥 Participação em guildas | **100% do time** | Top companies: 80% |
| 📈 Skills gap closure rate | **90%+ em 6 meses** | Referência: 12 meses |
| 🤖 AI Quotient (adoção real de AI tools) | **Alto** | KPI emergente 2026 |

---

## 📚 Referências e Frameworks

| Framework / Fonte | Contribuição |
|---|---|
| **World Economic Forum** | Future of Jobs Report 2025: 60% precisam de reskilling até 2027 |
| **DORA Metrics** | Métricas de elite em engineering performance |
| **Google G2G** | Modelo de mentoria peer-to-peer (Googler-to-Googler) |
| **Spotify Guild Model** | Comunidades de prática cross-squad |
| **ATD Research** | ROI de L&D: 218% revenue/employee |
| **LinkedIn Workforce Learning** | Shift para capability velocity |

---

## 🤝 Como Contribuir

Contribuições são bem-vindas! Se você quer sugerir melhorias, adicionar recursos ou trilhas de aprendizagem:

1. **Fork** este repositório
2. Crie uma **branch** com sua feature (`git checkout -b feature/nova-trilha`)
3. **Commit** suas mudanças (`git commit -m 'feat: adiciona trilha de Platform Engineering'`)
4. **Push** para a branch (`git push origin feature/nova-trilha`)
5. Abra um **Pull Request**

> 💡 **Dica:** Siga [Conventional Commits](https://www.conventionalcommits.org/) para mensagens de commit.

---

## 🔗 Nossos Outros Compromissos

<table>
  <tr>
    <td align="center">🌍</td>
    <td><a href="https://github.com/Ret-Consultoria/compromisso-impacto-social"><strong>Impacto Social</strong></a></td>
    <td>Open source, mentoria comunitária e Cascavel Security Framework</td>
  </tr>
  <tr>
    <td align="center">🌱</td>
    <td><a href="https://github.com/Ret-Consultoria/compromisso-sustentabilidade-ambiental"><strong>Sustentabilidade Ambiental</strong></a></td>
    <td>Green software, remote-first, carbon-aware computing</td>
  </tr>
  <tr>
    <td align="center">🤝</td>
    <td><a href="https://github.com/Ret-Consultoria/compromisso-diversidade-equidade-inclusao"><strong>Diversidade & Inclusão</strong></a></td>
    <td>Psych safety, bias-free hiring, pay equity audits</td>
  </tr>
  <tr>
    <td align="center">⚖️</td>
    <td><a href="https://github.com/Ret-Consultoria/compromisso-equilibrio-vida-pessoal"><strong>Equilíbrio Vida-Trabalho</strong></a></td>
    <td>Async-first, right to disconnect, wellbeing system</td>
  </tr>
</table>

---

<p align="center">
  <sub>
    <strong>RET Tecnologia</strong> · Engenharia de Software & Cibersegurança Ofensiva<br/>
    <a href="https://rettecnologia.org">rettecnologia.org</a> · <a href="mailto:contato@rettecnologia.org">contato@rettecnologia.org</a><br/>
    <em>A meia-vida de uma skill é 5 anos. Quem para de aprender, para de entregar. Nunca paramos.</em> 🚀
  </sub>
</p>
